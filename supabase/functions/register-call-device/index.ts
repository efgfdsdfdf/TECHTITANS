...
serve(async (request) => {
  ...
  if (!supportedProviders.includes(provider as typeof supportedProviders[number])) {
    return jsonResponse(request, 400, { error: "Unsupported call device provider" });
  }

  if (typeof payload.token !== "string" || payload.token.length < 20 || payload.token.length > 4096) {
    return jsonResponse(request, 400, { error: "A valid device push token is required" });
  }
  ...
  const deviceRecord = {
    ...
    device_type: defaultPlatform,
    browser: isAndroid ? "TechTitans Android APK" : "TechTitans iOS App",
  ...
  return jsonResponse(request, 200, { registered: true, provider });
});
